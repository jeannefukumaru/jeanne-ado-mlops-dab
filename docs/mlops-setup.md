...
[ML Config](../jeanne_ado_mlops_dab/databricks-resources/README.md)  guides to make changes to ML code or deployed jobs.
...
```
git add README.md docs .gitignore jeanne_ado_mlops_dab/databricks-resources/README.md
...
- **`jeanne-ado-mlops-dab-tests-ci.yml`**:<br>
  - **[CI]** Performs unit and integration tests<br>
    - Triggered on PR to main
- **`jeanne-ado-mlops-dab-bundle-cicd.yml`**:<br>
  - **[CI]** Performs validation of Databricks resources defined under `jeanne_ado_mlops_dab/databricks-resource`<br>
...
```
git add .azure/devops-pipelines/jeanne-ado-mlops-dab-bundle-cicd.yml .azure/devops-pipelines/jeanne-ado-mlops-dab-tests-ci.yml 
...
1. Create two separate Azure Pipelines under your Azure DevOps project using the ‘Existing Azure Pipelines YAML file’ option. One of these pipelines will use the `jeanne-ado-mlops-dab-tests-ci.yml` script, and the other will use the `jeanne-ado-mlops-dab-bundles-cicd.yml` script. See [here](https://docs.microsoft.com/en-us/azure/devops/pipelines/create-first-pipeline) for more details on creating Azure Pipelines.
1. Create a new variable group called `jeanne-ado-mlops-dab variable group` defining the following secret variables, for more details [here](https://learn.microsoft.com/en-us/azure/devops/pipelines/library/variable-groups?view=azure-devops&tabs=classic#create-a-variable-group):
...
1. Update both pipeline YAML files to use service connections rather than pipeline variables:
   - First, remove any lines where the environment variables are set in tasks in `jeanne-ado-mlops-dab-tests-ci.yml` or `jeanne-ado-mlops-dab-bundle-cicd.yml` files. Specifically, any lines where the following env vars are used: `PROD_AZURE_SP_TENANT_ID`, `PROD_AZURE_SP_APPLICATION_ID`, `PROD_AZURE_SP_CLIENT_SECRET`, `STAGING_AZURE_SP_TENANT_ID`, `STAGING_AZURE_SP_APPLICATION_ID`, `STAGING_AZURE_SP_CLIENT_SECRET`
...
5. Create two separate Azure Pipelines under your Azure DevOps project using the ‘Existing Azure Pipelines YAML file’ option. One of these Pipelines will use the `jeanne-ado-mlops-dab-tests-ci.yml` script, and the other will use the `jeanne-ado-mlops-dab-bundle-cicd.yml` script. See [here](https://docs.microsoft.com/en-us/azure/devops/pipelines/create-first-pipeline) for more details on creating Azure Pipelines.
...
Follow the instructions in [jeanne-ado-mlops-dab/databricks-resources/README.md](../jeanne_ado_mlops_dab/databricks-resources/README.md) to deploy ML resources
and production jobs.
...
[ML pull request guide](ml-pull-request.md) and [ML resource config guide](../jeanne_ado_mlops_dab/databricks-resources/README.md)  to propose, test, and deploy
