...
* Feature computation modules under `jeanne_ado_mlops_dab/feature_engineering` folder. 
...
* Python unit tests for feature computation modules in `jeanne_ado_mlops_dab/tests/feature_engineering` folder.
* Feature engineering notebook, `jeanne_ado_mlops_dab/feature_engineering/notebooks/GenerateAndWriteFeatures.py`, that reads input dataframes, dynamically loads feature computation modules, executes their `compute_features_fn` method and writes the outputs to a Feature Store table (creating it if missing).
...
* An automated integration test is provided (in `.github/workflows/jeanne-ado-mlops-dab-run-tests-fs.yml`) that executes a multi task run on Databricks involving the feature engineering and model training notebooks.
...
1. Implement your feature module, address TODOs in `jeanne_ado_mlops_dab/feature_engineering/features` and create unit test in `jeanne_ado_mlops_dab/tests/feature_engineering`
2. Update `jeanne_ado_mlops_dab/databricks-resources/feature-engineering-workflow-resource.yml`. Fill in notebook parameters for `write_feature_table_job`.
3. Update training data path in `jeanne_ado_mlops_dab/databricks-resources/model-workflow-resource.yml`.

We expect most of the development to take place in the `jeanne_ado_mlops_dab/feature_engineering` folder.

## Iterating on ML code

### Deploy ML code and resources to dev workspace using Bundles

Refer to [Local development and dev workspace](../jeanne_ado_mlops_dab/databricks-resources/README.md#local-development-and-dev-workspace)
...
* Run `databricks configure --profile jeanne-ado-mlops-dab-dev --token --host <your-dev-workspace-url>`, passing the URL of your dev workspace.
...
* From within the root directory of the current project, use the [dbx sync](https://dbx.readthedocs.io/en/latest/guides/python/devloop/mixed/#using-dbx-sync-repo-for-local-to-repo-synchronization) tool to copy code files from your local machine into the Repo by running
  `dbx sync repo --profile jeanne-ado-mlops-dab-dev --source . --dest-repo your-repo-name`, where `your-repo-name` should be the last segment of the full repo name (`/Repos/username/your-repo-name`)

#### Running code on Databricks
You can iterate on ML code by running the provided `jeanne_ado_mlops_dab/feature_engineering/notebooks/GenerateAndWriteFeatures.py` notebook on Databricks using
...
#### Prerequisites
* Python 3.8+
* Install feature engineering code and test dependencies via `pip install -I -r jeanne_ado_mlops_dab/requirements.txt -r test-requirements.txt` from project root directory.
